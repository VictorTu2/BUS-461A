...
# ```
# ```{r}
# studentBalance <- bank_full %>% filter(job == 'student') 
# studentBalance <- quantile(studentBalance$balance, c(0.75))['75%']
# 
# managementBalance <- bank_full %>% filter(job == 'management') 
# managementBalance <- quantile(managementBalance$balance, c(0.75))['75%']
# 
# bank_full <- bank_full %>% 
#     mutate(filterCondition = case_when((job == "student") & 
...
#                                        TRUE ~ 3)
#     )
# mgmtAndStudentsDF <- bank_full %>% filter((filterCondition == 1) | (filterCondition == 2))
# 
# boxplot(studentBalance ~ managementBalance, data = mgmtAndStudentsDF, col = "red")
# ```
